        ...
        {utilityItems?.map((item, idx) => {
          return (
            <section key={idx} className="flex flex-col">
              <div className="flex relative flex-col items-start px-20 pt-14 pb-5 w-full min-h-[700px] max-md:px-5 max-md:max-w-full">
                <CmsImage
                  loading="lazy"
                ...
                <div className="flex relative gap-5 items-start mt-96 min-h-[17px] max-md:mt-10">
                  {[0, 1, 2, 3].map((index) => (
                    <button
                      key={index}
                      aria-label={`Go to slide ${index + 1}`}
                      className={`flex shrink-0 w-4 h-4 rounded-full ${
                        index === 0 ? "bg-white" : "bg-teal-950"
                      }`}
                    />
                  ))}
                </div>
